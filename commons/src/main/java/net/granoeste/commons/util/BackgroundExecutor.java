...
public class BackgroundExecutor {
    ...
    public static final WrongThreadListener DEFAULT_WRONG_THREAD_LISTENER =
            new WrongThreadListener() {
                @Override
                public void onUiExpected() {
                    throw new IllegalStateException(
                            "Method invocation is expected from the UI thread");
                }
                ...
                public void onBgExpected(String... expectedSerials) {
                    if (expectedSerials.length == 0) {
                        throw new IllegalStateException(
                                "Method invocation is expected from a background thread, but it was called from the UI thread");
                    }
                    throw new IllegalStateException(
                            "Method invocation is expected from one of serials " +
                                    Arrays.toString(expectedSerials) +
                                    ", but it was called from the UI thread");
                }
                ...
                public void onWrongBgSerial(String currentSerial, String... expectedSerials) {
                    ...
                    throw new IllegalStateException(
                            "Method invocation is expected from one of serials " +
                                    Arrays.toString(expectedSerials) + ", but it was called from " +
        ...
        for (int i = TASKS.size() - 1; i >= 0; i--) {
            ...
            if (id.equals(task.id)) {
                if (task.future != null) {
                    ...
                    Log.w(TAG, "A task with id " + task.id +
                ...
                if (serial != null) {
                    ...
                    if (next != null) {
                        if (next.remainingDelay != 0) {
                            ...
                            next.remainingDelay = Math.max(0,
                                    (int) (targetTimeMillis - System.currentTimeMillis()));
                        }
    ...
    public interface WrongThreadListener {
         ...
         *
         * @param expectedSerials a list of allowed serials. If any background thread is
