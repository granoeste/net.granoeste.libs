...
public class Duration implements java.io.Serializable {
    private static final long serialVersionUID = 685422327082436810L;
    ...
    public Duration(boolean negative, Calendar calendar) throws
        ...
        if (years == 0 && months == 0 && days == 0 && hours == 0 &&
            minutes == 0 && seconds == 0) {
            throw new IllegalArgumentException("Bad Calendar For Duration");
        }
    }
    ...
    public void parseTime(String time) throws IllegalArgumentException {
        if (time.length() == 0 || time.indexOf("-") != -1) {
            throw new IllegalArgumentException("Bad Time Duration");
        }
    ...
    public void parseDate(String date) throws IllegalArgumentException {
        if (date.length() == 0 || date.indexOf("-") != -1) {
            throw new IllegalArgumentException("Bad Date Duration");
        }
    ...
    public long toMillis() {
        ...
        long time = 0;
        time += (int) (seconds * 100 - Math.round(seconds) * 100);
        time += (int) seconds * 1000;
        time += minutes * 60 * 1000;
        time += hours * 60 * 60 * 1000;
        time += days * 24 * 60 * 60 * 1000;
        time += months * (((365 * 4) + 1)/48) * 24 * 60 * 60 * 1000;
    ...
    /**
     * Returns duration in seconds. Truncate the milliseconds.
     *
